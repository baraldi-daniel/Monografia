...
for mensagem in consumidor:
    with open(r"C:\Users\baral\OneDrive\Documents\Engenharia de Dados - USP\Monografia\processamento-ram-cpu.txt", 'a') as f:
            print('The CPU usage is: '+str(psutil.cpu_percent())+'\t')
            print('RAM memory % used:'+str(psutil.virtual_memory()[2])+'\t')
            print('RAM Used (GB):'+str(psutil.virtual_memory()[3]/1000000000)+'\n\n')
            f.write('The CPU usage is: '+str(psutil.cpu_percent())+'\t'+'RAM memory % used:'+str(psutil.virtual_memory()[2])+'\t'+'RAM Used (GB):'+str(psutil.virtual_memory()[3]/1000000000)+'\n\n')
            f.flush()
